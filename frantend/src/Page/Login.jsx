...
const Login = () => {
  ...
  const { setLoading,x,setX} = useContext(AuthContext);
  ...
  const sendOtp = () => {
    ...
    axios
      .post(
       import.meta.env.VITE_API_URL+ "/api/otp_1",
  ...
  const login = () => {
   // Cookies.set('authToken', 'uerid457', { expires: 7 }); 
    ...
    setLoading(true);
    axios
      .post(import.meta.env.VITE_API_URL+"/api/login/v2", formData, {
        withCredentials: true,
      })
      .then((response) => {
        //alert(response?.data?.loginData?.UserId);
        setX(x => ({ ...x, userId: response?.data?.loginData?.userId }));
        setX(x => ({ ...x, name: response?.data?.loginData?.name }));
        setX(x => ({ ...x, phone: response?.data?.loginData?.phone }));
