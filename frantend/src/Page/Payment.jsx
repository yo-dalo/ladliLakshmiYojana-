...
function Payment() {
  const { plansId } = useParams()
      const {setLoading,x} = useContext(AuthContext);
...
const navigate = useNavigate();
  ...
  const [chackExistedPlans,setChackExistedPlans] = useState(false);
    ...
    try {
      let res = await axios.post(import.meta.env.VITE_API_URL+"/api/payment/test/v5",{plansId},{ withCredentials: true });
     ...
     await setOrderIddd(res.data.order_id);
      ...
      seterrrr(error.response.data.message);
       await myToast.info(error.response.data.message)
  ...
  const verifyPayment = async (odId,pyId) => {
    try {
      let res = await axios.post(import.meta.env.VITE_API_URL+"/verify4", {
        orderIddd: odId,
        paymentId:pyId
  ...
  const handleClick = async (e) => {
    ...
    const getplansdetails = (plansId)=>{
        axios.get(import.meta.env.VITE_API_URL+`/api/show/plans/${plansId}`,{ withCredentials: true })
            .then((response) => {
